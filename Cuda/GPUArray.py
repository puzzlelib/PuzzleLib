...
from PuzzleLib.Cuda.Driver import GPUArray
...
def memoizeOnCtx(fn):
	...
	def memoizer(*args):
		ordinal = Driver.Device.getCurrent()
		deviceCache = cache.get(ordinal, None)

		if deviceCache is None:
			deviceCache = {}
			cache[ordinal] = deviceCache

		obj = deviceCache.get(args, None)
		if obj is not None:
			return obj

		obj = fn(*args)
		deviceCache[args] = obj
...
def getMinMaxKernel(dtype):
	from PuzzleLib.Cuda.SourceModule import dtypeToCtype, ReductionKernel
	ctype = dtypeToCtype[dtype.type]
	...
	minKer = ReductionKernel(
		outtype=dtype.type, neutral=maxval, reduceExpr="min(a, b)", mapExpr="data[i]", arguments=arguments, name="min"
	)

	maxKer = ReductionKernel(
		outtype=dtype.type, neutral=minval, reduceExpr="max(a, b)", mapExpr="data[i]", arguments=arguments, name="max"
	)
